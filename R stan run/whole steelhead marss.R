...
x1 <- model.matrix(~-1+seals+npgo,data=enviro)

XXvars <- c("total_rain_run","mean_temp_run","log_adults")
...
fit <- stan(file="Stan code/steelhead dlm.stan",data=dat, iter=8000,chains=6,cores=6,control=list("adapt_delta"=0.99,"max_treedepth"=15))
...
summary(fit,pars=c("s0","a0","r0","x0","sigma_surv_pro","sigma_surv_obs","sigma_adult_obs","sigma_run_obs","sigma_rec_process","sigma_rec_obs"),probs=c(0.025,0.975))$summary
...
matLayout <- matrix(0,nrow=12,ncol=12)
matLayout[1:3,1:9] <- 1
...
points(1/(1+exp(-sh_annual$logit_surv)),pch=21,bg="orange")
ppd <- extract(fit)$y1_miss
ci <- quantile(1/(1+exp(-extract(fit)$y1_miss)),probs=c(0.05,0.95))
segments(x0=1,y0=ci[1],y1=ci[2],lwd=2,col="darkorange")
points(1,mean(1/(1+exp(-ppd))),pch=21,bg="darkorange")
