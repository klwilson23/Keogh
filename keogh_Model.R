model {
  ...
  #  muStock[1,j] <- recruits[1,j]*marSurv_t[1,j]
  #  for(i in 2:Nyears)
  #  {
  #    stock[i,j] ~ dnorm(muStock[i,j],1/(muStock[i,j]*stock.cv[j]))T(0,)
  #    muStock[i,j] <- muRec[i-1,j]*marSurv_t[i-1,j]
  #  }
  #} # end the data portion
  ...
  for(j in 1:Nspecies) # the regressions
  {
    ...
    for(i in 2:Nyears)
    {
      ...
      la_t[i,j] ~ dnorm(log(a[j]),tau.la[j])
      a_t[i,j] <- exp(la_t[i,j])
      #b_t[i,j] ~ dnorm(b[j]+beta[j,1]*obsRec[i-1,1]+beta[j,2]*obsRec[i-1,2]+beta[j,3]*obsRec[i-1,3]+beta[j,4]*obsRec[i-1,4]+beta[j,5]*obsRec[i-1,5],tau.b[j])T(0,)
      b_t[i,j] ~ dnorm(b[j],tau.b[j])T(0,)
      # marine survival from smolt to adult is a logit regression
  ...
  for(j in 1:Nspecies)
  {
    sd.surv[j] ~ dhalfcauchy(5) # variance in marine survival
    ...
    tau.la[j] ~ dhalfcauchy(5) # variance in productivity
    tau.b[j] ~ dhalfcauchy(5) # variance in DD-survival slope
    stock.cv[j] ~ dhalfcauchy(5) # cv in recruitment
    rec.cv[j] ~ dhalfcauchy(5) # cv in recruitment
    b[j] ~ dnorm(1e-3,1e-3)T(0,) # DD-survival slope
    a[j] ~ dnorm(10,1e-3)T(0,) # intercept
  }
